...
class auto_word_wind_res(models.Model):
    ...
    case_name = fields.Char(u'方案名称(结果)', required=True, default="方案1结果")
    ...
    Turbine=fields.Char(string=u'风机', readonly=True)
    tur_id=fields.Char(string=u'风机ID', readonly=True)
    X=fields.Char(string=u'X', readonly=True)
    Y=fields.Char(string=u'Y', readonly=True)
    Z=fields.Char(string=u'Z', readonly=True)
    H=fields.Char(string=u'计算高度', readonly=True)
    Latitude=fields.Char(string=u'经度', readonly=True)
    Longitude=fields.Char(string=u'纬度', readonly=True)
    TrustCoefficient=fields.Char(string=u'信任系数', readonly=True)
    WeibullA=fields.Char(string=u'A', readonly=True)
    WeibullK=fields.Char(string=u'K', readonly=True)
    EnergyDensity=fields.Char(string=u'能量密度', readonly=True)
    PowerGeneration=fields.Char(string=u'理论电量', readonly=True)
    PowerGeneration_Weak=fields.Char(string=u'尾流后理论电量', readonly=True)
    CapacityCoe=fields.Char(string=u'CapacityCoe', readonly=True)
    AverageWindSpeed=fields.Char(string=u'平均风速', readonly=True)
    TurbulenceEnv_StrongWind=fields.Char(string=u'强风状态下平均环境湍流强度', readonly=True)
    Turbulence_StrongWind=fields.Char(string=u'强风状态下的平均总体湍流强度', readonly=True)
    AverageWindSpeed_Weak=fields.Char(string=u'考虑尾流效应的平均风速', readonly=True)
    Weak=fields.Char(string=u'尾流效应导致的平均折减率', readonly=True)
    AirDensity=fields.Char(string=u'该点的空气密度', readonly=True)
    WindShear_Avg=fields.Char(string=u'平均风切变指数', readonly=True)
    WindShear_Max=fields.Char(string=u'最大风切变指数', readonly=True)
    WindShear_Max_Deg=fields.Char(string=u'最大风切变指数对应方向扇区', readonly=True)
    InflowAngle_Avg=fields.Char(string=u'绝对值平均入流角', readonly=True)
    InflowAngle_Max=fields.Char(string=u'最大入流角', readonly=True)
    InflowAngle_Max_Deg=fields.Char(string=u'出现最大入流角的风向扇区', readonly=True)
    NextTur=fields.Char(string=u'最近相邻风机的标签', readonly=True)
    NextLength_M=fields.Char(string=u'相邻风机的最近距离', readonly=True)
    Diameter=fields.Char(string=u'叶轮直径', readonly=True)
    NextLength_D=fields.Char(string=u'以叶轮直径为单位的相邻风机最近距离', readonly=True)
    NextDeg=fields.Char(string=u'最近相邻风机的方位角', readonly=True)
    Sectors=fields.Char(string=u'扇区数量', readonly=True)
